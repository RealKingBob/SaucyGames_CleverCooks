...
local CoinCounterUI = Knit.CreateController { Name = "CoinCounterUI" }
...
local CoinContainer = PlayerGui:WaitForChild("CoinCollectorGui");
local CoinPrefab = PlayerGui:WaitForChild("Prefabs"):WaitForChild("CoinImage");

local LeftBar = PlayerGui:WaitForChild("LeftBar");
local CoinCounter =  LeftBar:WaitForChild("MainContainer"):WaitForChild("Coins");
local CoinsLabel = CoinCounter:WaitForChild("Main"):WaitForChild("Main"):WaitForChild("TextLabel");
local AmountLabel = CoinCounter:WaitForChild("Amount");
local AddButton = LeftBar.MainContainer.Coins:WaitForChild("Add"):WaitForChild("TextButton");
...
function CoinCounterUI:CollectCoins(Amount, AmountAdded)
	--print(Amount)
    local CoinDrop = Sfx:WaitForChild("CoinPop"):Clone() do
		...
		task.delay(.5 * math.random(), function()
            if (i == TargetCoins) then
                self:SpawnDrop(function()
					if (AmountAdded) then
						local Tween = TweenService:Create(AmountLabel, CollectTextInfo, { Position = UDim2.fromScale(0.4, 0.25) })
						Tween:Play();
...
function CoinCounterUI:SpawnDrop(DropCb)
	local DropEnded = Instance.new("BindableEvent");
	local ScreenSize = CoinContainer.AbsoluteSize;
	...
	FadeIn.Completed:Connect(function()
		...
		local TargetVector2 = CoinCounter.AbsolutePosition + (CoinCounter.AbsoluteSize / 2);
...
function CoinCounterUI:Expand()
    local Container = CoinCounter;
...
function CoinCounterUI:Update(currentCoins)
	...
	if currentCoins ~= nil then
		local Tween = TweenModule.new(ChangeInfo, function(Alpha)
			...
			CoinsLabel.Text = NumberSuffix(CurrentCoins);
		end)
...
function CoinCounterUI:KnitStart()
...
function CoinCounterUI:KnitInit()
