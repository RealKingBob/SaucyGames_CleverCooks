...
function GameRound:Update(gameMode)
    ...
    local NumOfParticipants = #CollectionService:GetTagged(Knit.Config.ALIVE_TAG)
    ...
    for _, player : Player in pairs(CollectionService:GetTagged(Knit.Config.ALIVE_TAG)) do
        ...
        task.spawn(function()
            ...
            if tonumber(PlayerRank) >= NumOfParticipants * Knit.Config.REGULAR_THRESHOLD then
                CollectionService:AddTag(player, Knit.Config.GHOST_TAG);
            end
        end)
        task.wait(0.02)
    end

    print("[GameRound][RACE]: Race round ended")
