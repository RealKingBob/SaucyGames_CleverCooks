...
function GameRound:Update(gameMode)
    ...
    local Threshold = #CollectionService:GetTagged(Knit.Config.ALIVE_TAG) * Knit.Config.REGULAR_THRESHOLD;

    for timeLeft = self.TimeLeft, 0, -1 do
        GameService.Client.TimeLeftSignal:FireAll(timeLeft, gameMode)
        if #CollectionService:GetTagged(Knit.Config.DUCK_TAG) * Knit.Config.REGULAR_THRESHOLD < Threshold and not SinglePlayer then
            break;
        end
    ...
    for _, player : Player in pairs(CollectionService:GetTagged(Knit.Config.ALIVE_TAG)) do
        ...
        task.spawn(function()
            for index, data in pairs(PlayerTimes) do
                ...
                if tonumber(index) >= Threshold then
                    CollectionService:AddTag(data.Player, Knit.Config.GHOST_TAG);
                end
            end
        end)
        task.wait(0.02)
