--[[
    Name: Infection Round [V2]
    By: Real_KingBob
    Created: 12/10/21 
    Updated: 2/1/21
    Description: This module handles the INFECTION round game mode
]]
...
GameRound.HunterTip = "ELIMATE ALL DUCKS TO WIN!"
GameRound.DuckTip = "REACH TO THE END OF THE MAP WITHOUT DYING!"
...
function GameRound.new(length, gameMode, mapName)
    print("[GameRound][INFECTION]: Infection round created")
    GameService = Knit.GetService("GameService");
    ...
    if mapName then
        ...
        if MapLocation then
            print("[GameRound][INFECTION]: Map selected")
            ...
            local NumOfHunters = 1;--// Calculate number of hunters there should be for this round
            ...
            print("[GameRound][INFECTION]: Picked hunters")
            ...
            print("[GameRound][INFECTION]: Adding ducks")
            ...
            for _, player in pairs(CollectionService:GetTagged(Knit.Config.ALIVE_TAG)) do
                ...
                self._maid:GiveTask(humanoid.Died:Connect(function()
                    if not CollectionService:HasTag(player, Knit.Config.HUNTER_TAG) then
                        ...
                        self:AddHunter(player);
                        player.RespawnLocation = self.hunterSpawn;
                        CollectionService:RemoveTag(player, Knit.Config.DUCK_TAG)
                        GameService:SetCurrentHunter(player);
                    end
                end))
            end

            print("[GameRound][INFECTION]: Set up checkpoints and endpoint")
            self:SetupCheckpoints();
            self:SetupEndpoint();

            print("[GameRound][INFECTION]: Checking if current map exists if not then wait")
            repeat task.wait(0.5) until #workspace.CurrentMap:GetChildren() > 0
            print("[GameRound][INFECTION]: Found map, starting music and game")
            ...
            task.wait(2)
            print("[GameRound][INFECTION]: Teleporting ducks and hunters to the map")
            ...
            --// NOTE: Starts cutscene for map
            print("[GameRound][INFECTION]: Starting cutscene for players")

            local forceStartGame = false
            task.spawn(function() task.wait(25) forceStartGame = true end)
            repeat task.wait(0) until #CollectionService:GetTagged(Knit.Config.CUTSCENE_TAG) == 0 or forceStartGame == true
            ...
            warn("[GameRound][INFECTION]: Map module not found!");
            self.Finished:Fire();
        end
    else
        warn("[GameRound][INFECTION]: There no more maps");
        self.Finished:Fire();
    end
...
function GameRound:Update(gameMode)
    print("[GameRound][INFECTION]: Updating game now")
    GameService = Knit.GetService("GameService");
    MapProgessService = Knit.GetService("MapProgessService");
    AudioService = Knit.GetService("AudioService");
    local HunterService = Knit.GetService("HunterService")
    ...
    print("[GameRound][INFECTION]: Starting to track players")
    Players.RespawnTime = 2;
    self:StartTracking();

    local MapLength = MapProgress:GetMapLength(PathDataPoints)
    print("[GameRound][INFECTION]: Starting timer")
    for timeLeft = self.TimeLeft, 0, -1 do
        ...
        for _, player : Player in pairs(CollectionService:GetTagged(Knit.Config.ALIVE_TAG)) do
            ...
            --[[
            if timeLeft <= (self.TimeLeft * (2/3)) then
                if CollectionService:HasTag(player, Knit.Config.IDLE_TAG) == false then
                    print(MapProgress:GetPlayerProgress(player)[1])
                end
            end
            print(MapProgress:GetPlayerProgress(player)[1])]]
    ...
    print("[GameRound][INFECTION]: Timer ended, evaluating player")
    for _, player : Player in pairs(CollectionService:GetTagged(Knit.Config.ALIVE_TAG)) do
    ...
    print("[GameRound][INFECTION]: INFECTION round ended")
