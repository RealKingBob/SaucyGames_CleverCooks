...
function Environment:SetLighting()
    ...
    local LightingInfo = MapSettings.Candyland.Lighting;
...
function Environment:SetMap()
    ...
    local MapObject = ServerStorage.Maps["Candyland"];
   -- print("[Candlyland/Environment]:","Setting Up Map!");
    if CurrentMap then
    ...
    --// Islands Sections
    local MainSection = {"Stage 5 [CHAINS]"}; -- "Stage 5 [CANNON]", "Stage 5 [CHOCOLATE]", "Stage 5 [CHAINS]",
...
function Environment:ClientEnvironment(MapObject)
    ...
    if CurrentMap and MapObject then
        --print("[Candyland/Environment]:","Connecting Client's Environment!");
