...
function Environment:SetLighting()
    ...
    local LightingInfo = MapSettings.Kingdom.Lighting;
...
function Environment:SetMap()
    ...
    local MapObject = ServerStorage.Maps["Kingdom"];
    print("[Kingdom/Environment]:","Setting Up Map!");
    if CurrentMap then
    ...
    --// Islands Sections
    local MainSection = {"Stage 3 (BATTERING RAMS)", "Stage 3 (DRAGON)", "Stage 3 (MORTARS)"};
