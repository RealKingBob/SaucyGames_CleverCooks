...
function Path:Run(target)
	...
	--Checks if running wait until reached waypoint
	if self._status ~= Path.StatusType.Idle then
		repeat wait() until self._waypointReached == true;
	end
