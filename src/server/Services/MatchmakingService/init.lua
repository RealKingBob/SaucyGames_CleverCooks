...
function updateQueue(map, ratingType, stringRoundedRating)
	...
	if not success then
		...
		warn(errorMessage)
		return updateQueue(map, ratingType, stringRoundedRating)
	end
		...
		task.spawn(function()
			...
			if isMain and not CLOSED then
				mainMemory:UpdateAsync("MainJobId", function(old)
					if old == nil or old == "" or old[1] == mainJobId then
	...
	task.spawn(function()
		...
		while not Service.IsGameServer and not CLOSED do
			...
			if lastCheckMain + 10000 <= now then
				...
				if mainJobId ~= nil and mainJobId[1] == game.JobId then
					local success, newHighBid = pcall(function() 
						mainMemory:UpdateAsync("MainJobId", function(old)
							--print("Old",old, "[JobId", game.JobId, "]", CLOSED, now)
							if (old == nil or old == "" or old[1] == game.JobId) and not CLOSED then
								return {game.JobId, now}
							end
							return nil
						end, 86400)
			...
			if mainJobId == nil or mainJobId[2] + 30000 <= now then
				local success, newHighBid = pcall(function() 
					mainMemory:UpdateAsync("MainJobId", function(old)
						if (old == nil or old == "" or mainJobId == nil or old[1] == mainJobId[1]) and not CLOSED then
							return {game.JobId, now}
						end
						return nil
					end, 86400)
				...
				for i, map in ipairs(queuedMaps) do
					...
					for ratingType, skillLevelAndQueue in pairs(mapQueue) do
						...
						for skillLevel, queue in pairs(skillLevelAndQueue) do
							...
							--print(Service.PlayerRanges[map])
...
function MatchmakingService:GetAllRunningGames()
	...
	local toReturn = {}
	while true do
		local setSuccess, runningGames = pcall(function()
			return runningGamesMemory:GetRangeAsync(Enum.SortDirection.Ascending, 200, bound)
		end)
		
		--print(runningGames)
		
		if setSuccess == false then
			break
		end
