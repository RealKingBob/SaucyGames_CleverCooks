...
function AvatarService:GetAvatarHat(Player) -- [Player : Instance]
    local PlayerData = PlayerProfileData[Player] or Knit.GetService("DataService"):GetProfile(Player)
    if PlayerData then
...
function AvatarService:SetRandomAvatarHat(Player)
    local PlayerData = Knit.GetService("DataService"):GetProfile(Player)
    if PlayerData then
...
function AvatarService:SetAvatarHat(Player, HatName)
    ...
    if not PlayerProfileData[Player] then
        PlayerData = Knit.GetService("DataService"):GetProfile(Player)
    else
    ...
    if PlayerData then 
        ...
        if PlayerData.Data.Inventory.CurrentHat and HatName then
            if PlayerData.Data.Inventory.CurrentHat ~= HatName then
                ...
                Knit.GetService("InventoryService"):InventoryChanged(Player)
            end
    ...
    if PlayerData then 
        ...
        if PlayerData.Data.Inventory.CurrentBoosterEffect and BoostName then
            ...
            if BoosterEffects:FindFirstChild(tostring(BoostName)) then
                ...
                newSound.SoundId = game:GetService("ReplicatedStorage").GameLibrary.Sounds.BoostEffects:FindFirstChild(tostring(BoostName)).SoundId
