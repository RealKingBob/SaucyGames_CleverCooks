...
local TargetIndicatorGuiManager = require(Libraries:WaitForChild("TargetIndicatorGuiManager"))
local DamageBillboardHandler = require(Libraries:WaitForChild("DamageBillboardHandler"))

local WeaponsSystemGuiTemplate = ReplicatedStorage:WaitForChild("Assets"):WaitForChild("WeaponsSystemGui")
...
function WeaponsGui.new(weaponsSystem)
	...
	coroutine.wrap(function()
		...
		self.TargetIndicatorGuiManager = TargetIndicatorGuiManager.new(self)
		...
		self.ammoFrame = self.gui:WaitForChild("Ammo")
		self.AmmoText = self.ammoFrame:WaitForChild("AmmoText")
		...
		WeaponData.OnClientEvent:Connect(function(cmd, otherPlayerPosition)
			...
			elseif cmd == "ClosestTarget" then
				--self.TargetIndicatorGuiManager:ActivateDirectionalIndicator("TargetIndicator", otherPlayerPosition)
			end
		end)
...
function WeaponsGui:setEnabled(enabled)
	...
	if self.enabled and CollectionService:HasTag(LocalPlayer, "Hunter") then
	...
	if self.gui then
		if self.enabled == true and CollectionService:HasTag(LocalPlayer, "Hunter") then
...
function WeaponsGui:setAmmoEnabled(ammoEnabled)
	if self.ammoEnabled == ammoEnabled then
		return
	end

	self.ammoEnabled = ammoEnabled
	if self.ammoFrame then
		self.ammoFrame.Visible = self.ammoEnabled
	end
end

function WeaponsGui:setAmmoText(ammoText)
	if self.ammoFrame and self.AmmoText then
