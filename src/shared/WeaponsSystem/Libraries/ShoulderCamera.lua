...
function ShoulderCamera.new(weaponsSystem)
	...
	self.mouseRadsPerPixel = Vector2.new(1 / 300, 1 / 300)--Vector2.new(1 / 480, 1 / 480)
...
function ShoulderCamera:setEnabled(enabled)
	...
	--print("shoulderCam:", self.enabled, #self.eventConnections, workspace.CurrentCamera.CameraSubject, workspace.CurrentCamera.CameraType)
	if self.enabled and #self.eventConnections == 0 then -- and CollectionService:HasTag(LocalPlayer, "Hunter")
		...
		--print("Scriptable",self.currentCamera,workspace.CurrentCamera)
...
function ShoulderCamera:onCurrentCharacterChanged(character)
	...
	if self.currentCharacter then
		...
		if self.currentRootPart.Parent:FindFirstChild("Sphere") then
			return
		end
