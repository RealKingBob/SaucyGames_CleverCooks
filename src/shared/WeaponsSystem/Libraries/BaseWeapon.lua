...
function BaseWeapon:doInitialSetup()
	...
	if selfClass.CanBeReloaded then
		if IsServer then
			...
			if weaponsGui then
				weaponsGui:setAmmoText(tostring(self.ammoInWeaponValue.Value).."/"..tostring(self:getConfigValue("AmmoCapacity", 30)))
			end
		else
...
function BaseWeapon:getAnimationController()
	...
	if self.instanceIsTool then
		--print("self.instance.Parent", self.instance.Parent)
