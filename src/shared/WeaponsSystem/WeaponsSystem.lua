...
function WeaponsSystem.setWeaponEquipped(weapon, equipped)
	...
	if WeaponsSystem.camera then
		...
		if WeaponsSystem.currentWeapon then
			local WeaponModel = WeaponsSystem.currentWeapon["instance"];
			if workspace.Terrain:FindFirstChild(Players.LocalPlayer.Name) then
				local Beam = workspace.Terrain:FindFirstChild(Players.LocalPlayer.Name).Beam
				Beam.Enabled = false
			end
...
local function _defaultDamageCallback(system, target, amount, damageType, dealer, hitInfo, damageData)
	...
	if target.Parent then
		if target.Parent:FindFirstChildWhichIsA("Humanoid") then
			...
			target = target.Parent:FindFirstChildWhichIsA("Humanoid")
		elseif target.Parent.Parent:FindFirstChildWhichIsA("Humanoid") then
			Character = target.Parent.Parent;
			target = target.Parent.Parent:FindFirstChildWhichIsA("Humanoid")
		else
			Character = nil;
			target = nil;
		end
		if target then
			...
			if not CollectionService:HasTag(player, "Hunter") then
				if target:IsA("Humanoid") then
					if Character:FindFirstChild("KillerTag") then
						Character:FindFirstChild("KillerTag").Value = dealer;
					else
						local killerTag = Instance.new("ObjectValue")
						killerTag.Name = "KillerTag";
						killerTag.Value = dealer;
						killerTag.Parent = Character;
					end
				end
...
local function _defaultTargetCallback(system, target, amount, damageType, dealer, hitInfo, damageData)
	--print(system, target, amount, damageType, dealer, hitInfo, damageData)
	if CollectionService:HasTag(target, "Target") and not CollectionService:HasTag(target, "InitiateTrap") then
...
function WeaponsSystem.doDamage(target, amount, damageType, dealer, hitInfo, damageData)
	...
	if IsServer then
		...
		if CollectionService:HasTag(target, "Target") then
			local handler = _defaultTargetCallback
			WeaponData:FireClient(dealer, "ClosestTarget", target.CFrame.Position)
